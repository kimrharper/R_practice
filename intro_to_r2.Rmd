...
```{r assfo-feb-flights-arrival-delays-carrier}
sfo_feb_flights %>%
  group_by(carrier) %>%
  summarise(mean_ad = mean(arr_delay), sd_ad = sd(arr_delay), iqr_ad = IQR(arr_delay), n = n())
```
...
nycflights %>%
  group_by(month) %>%
  summarise(mean_ad = mean(arr_delay), sd_ad = sd(arr_delay), iqr_ad = IQR(arr_delay), 
            mean_dd = mean(dep_delay), n = n()) %>%
  arrange(desc(mean_dd))
```
...
nycflights %>%
  group_by(origin) %>%
  summarise(ot_dep_rate = sum(dep_delay) / n()) %>%
  arrange(desc(ot_dep_rate))
```
...
nycflights <- nycflights %>%
  mutate(air_speed=air_time/distance)

avg_speed_df <- nycflights %>%
  group_by(tailnum) %>%
  summarise(avg_speed = mean(air_speed), avg_distance = mean(distance)) %>%
...
avg_speed_check <- nycflights %>%


ggplot(data=avg_speed_df, aes(avg_speed,avg_distance)) +
  geom_point()
```
...
on_time_df <- nycflights %>%
  mutate(arr_type = ifelse(arr_delay <= 0, "on time", "delayed"), 
         dep_type = ifelse(dep_delay < 5, "on time", "delayed"))


sum(on_time_df$dep_type == "delayed") / sum(on_time_df$arr_type == "on time")
